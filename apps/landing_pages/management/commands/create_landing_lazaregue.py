"""
Crée la landing LAZARÈGUE AVOCATS (Sarah Hinderer) à partir du JSON existant.
Usage : python manage.py create_landing_lazaregue
Idempotent : n'écrase pas si la landing existe déjà (met à jour le content_json si --update).

Stratégie : Niche / Argent — appui technique via scripts, structuration données RGPD/conformité.
Contact : Sarah Hinderer. Angle : « Mission d'appui technique » pour un dossier complexe.
"""
import json
...
class Command(BaseCommand):
    help = "Crée la landing LAZARÈGUE AVOCATS (Sarah Hinderer) depuis docs/contacts/lazaregue-avocats/landing-proposition-sarah.json"
    ...
    def handle(self, *args, **options):
        json_path = (
            ...
            / "contacts"
            / "lazaregue-avocats"
            / "landing-proposition-sarah.json"
        )
        ...
        title = content.get("page_title") or "Optimisation Growth pour LAZARÈGUE AVOCATS"
        slug = "lazaregue-avocats"
        prospect_company = "LAZARÈGUE AVOCATS"
        prospect_name = "Sarah Hinderer"
        sector = "juridique"
