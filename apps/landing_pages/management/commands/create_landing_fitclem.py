"""
Crée la landing FitClem (candidature Responsable Marketing Digital) à partir du JSON existant.
Usage : python manage.py create_landing_fitclem [--publish]
Idempotent : n'écrase pas si la landing existe déjà (met à jour le content_json si --update).

Contexte : LPPP-FitClem — Candidature Clémentine Sarlat / FitClem, Paris. Livrable multipage (étude marketing, SEO, graphique, KPI).
"""
import json
...
class Command(BaseCommand):
    help = "Crée la landing FitClem (candidature RMD) depuis docs/contacts/fitclem/landing-proposition-fitclem.json"
    ...
    def handle(self, *args, **options):
        json_path = (
            ...
            / "contacts"
            / "fitclem"
            / "landing-proposition-fitclem.json"
        )
        ...
        title = content.get("page_title") or "Lucas Tymen — Candidature Responsable Marketing Digital FitClem"
        slug = "fitclem"
        prospect_company = "FitClem"
        prospect_name = "Clémentine Sarlat"
        sector = "autre"
