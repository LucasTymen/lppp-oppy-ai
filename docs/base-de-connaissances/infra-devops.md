...
|-------|----------------|------------------|
| **Vercel** | **Frontend Next.js** (landing pages) — déploiement natif Next.js, liaison Git, déploiement auto. **Espace projet** : [vercel.com/lucas-tymens-projects](https://vercel.com/lucas-tymens-projects). Voir `stack-frontend-nextjs-react.md`. **Sécurité** : suivre **`plan-mise-a-jour-nextjs-securite.md`** pour les advisories RSC (upgrade `next` obligatoire, inventaire de tous les repos). | Env vars dans le dashboard Vercel ; token CLI créé dans Vercel (Account → Tokens), jamais dans le dépôt |
