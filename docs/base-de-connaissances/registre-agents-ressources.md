...
> **⚠️ Mesures anticrash obligatoires (2026-02-21)** : système instable — **tous les agents** appliquent systématiquement : commit après chaque bloc logique, mise à jour logs (log-projet, log-ia) à chaque étape, `.progress-<slug>.md` pour tâches multi-étapes, reprise au dernier marqueur en cas de crash. Voir `procedure-anti-crash.md`, `pilotage-agents.mdc`.
...
| 11 | **Infographiste / Data Viz** (infographies dynamiques et animées HTML/CSS/JS, effet waouh) | `agents-roles-responsabilites.md` § Infographiste | `pilotage-agents.mdc`, `infographiste-dataviz.mdc` |
...
| `infographiste-dataviz.mdc` | **InfographicCraft** : infographies type Pictochart (un seul fichier HTML autonome, vanilla, print-friendly) ; **assiste Chef de Projet et Rédacteur** pour rendre les pages dynamiques en résumant les chiffres clés ; brief THEME/AUDIENCE/TON/SECTIONS/DATA/CONSTRAINTS → plan puis `infographie.html` ; pas d’invention de chiffres | **Infographiste** | Non |
...
### Infographiste / Data Viz (InfographicCraft)

- **Rôle** : **Assister le Chef de Projet et le Rédacteur** pour rendre les pages plus dynamiques et intéressantes en résumant les **chiffres clés** par des infographies (un seul fichier HTML autonome, style Pictochart).
- **Espace de travail** : `docs/contacts/<slug>/` (ex. `infographie-audit-seo.html`) ; réutilisables dans `docs/base-de-connaissances/`.
- **Règle** : `.cursor/rules/infographiste-dataviz.mdc`
- **Brief** : Thème, Audience, Ton, Sections obligatoires, Données/chiffres (DATA), Contraintes. Si DATA vide → emplacements « À compléter » sans inventer.
- **Sortie** : (1) Plan court (structure + composants), (2) contenu complet de `infographie.html` (HTML+CSS+JS inline, vanilla, print-friendly, accessibilité, prefers-reduced-motion).
...
- **Attentes contenu et enrichissement (référence unique)** : `docs/base-de-connaissances/attentes-contenu-et-enrichissement-qualite.md` — ce qu’on attend en contenu (qualité P4S, vivant, éditorial) et en qualité d’enrichissement (une tâche = un outil, pas d’hallucination, rôle conseiller) ; pour ne pas recommencer, s’y tenir.
...
| `strategie-automatisation-correspondances-prospection.md` | **Correspondances cibles ↔ audits SEO ↔ prospects** : flux (JSON lowtech → audit SEO → dossier contact → prospects), mapping besoin ↔ preuve, convention slug, checklist, script optionnel liste/CSV. Pilotes : toi (audits, prospection), Expert SEO, Growth, Automatizer. | **Growth**, Expert SEO, **Automatizer**, Chef de Projet |
| `protection-donnees-et-sauvegardes.md` | **Protection des données** : tous les agents non destructifs ; pas de `down -v` ni réinitialisation sans sauvegarde et accord explicite. Sauvegarde PostgreSQL : `make backup` (user/base depuis .env) ; workflows : voir `sauvegarde-workflows-flowise-n8n.md`. Réf. `pilotage-agents.mdc` § Agents non destructifs. | **Tous les agents**, **DevOps**, Chef de Projet |
...
| `cartographie-pages-casapy-paralleles.md` | **Cartographie DevOps** : deux pages Casapy (A = Django dynamique localhost:8010/p/casapy/, B = export statique deploy/LPPP-Casapy pour Vercel) — comment les distinguer, éviter confusion cache / modifs pas visibles | **DevOps**, Dev Django |
...
| `segmentations/2026-01-30-feature-chatbot-landing-agents-municipaux.md` | **Feature chatbot fonctionnel landing agents municipaux** : rendre le chatbot opérationnel sur /p/maisons-alfort/ ; stratégies **non destructives** (pas de reset Flowise/FAISS). Équipe : **Assistant** (coordination, doc), **Architecte** (chaîne d'appel, points de défaillance), **Automatizer** (Flowise, embed, flux), **Pentester** (iframe, CSP, sécurité). Mécanisme réutilisable pour autres landings et bots scraping. | **Assistant**, **Architecte**, **Automatizer**, **Pentester** |
| `segmentations/2026-01-30-sprint-deploiement-stack-contabo-ssh-landing-chatbot.md` | **Sprint déploiement stack complète Contabo** : déployer la stack LPPP, activer clés SSH, s'assurer qu'aucun chevauchement avec SquidResearch, raccorder landing municipale + chatbot. **Orchestrateur** pilote ; **protecteur et non destructif** (priorité SquidResearch). DevOps, Architecte, Automatizer, Chef de Projet, Pentester. | **Orchestrateur**, **DevOps**, **Architecte**, **Automatizer**, **Chef de Projet**, **Pentester** |
| `segmentations/2026-02-controle-flux-et-dependances-python.md` | **Contrôle flux + interférences Python/dépendances** : faire contrôler par DevOps (flux, services, check_flowise_embed), Dev Django (pip check, pytest, chaîne vue→template), Pentester (secrets, CORS). Résultats exécution : pip check OK, Django check OK, 28 tests passent, aucun secret dans les templates. | **DevOps**, **Dev Django**, **Pentester** |
| `segmentations/2026-02-06-sprint-controle-flux-chatbot-landing-maisons-alfort.md` | **Sprint contrôle des flux chatbot** : page visible mais zone chat vide ; stratégies et tests de contrôle des flux. **DevOps** (ports, .env, santé Flowise, curl prediction), **Architecte** (chaîne intégration, points de défaillance), **Ingénieur système** (réseau, processus, checklist), **Pentester** (CORS, CSP, X-Frame-Options, secrets). | **DevOps**, **Architecte**, **Ingénieur système**, **Pentester**, **Chef de Projet** |
| `segmentations/2026-02-06-demande-agents-chatbot-landing-pourquoi-comment.md` | **Demande à tous les agents** : « Pourquoi ça ne marche pas ? » et « Comment en finir pour utiliser le chatbot complètement dans la landing ? » — chaque rôle (DevOps, Architecte, Ingénieur système, Pentester, Dev Django, Automatizer, Designer, Chef de Projet) est invité à répondre et à documenter. | **Tous les agents concernés**, **Chef de Projet** (synthèse) |
| `segmentations/2026-02-20-repartition-agents-rapports-seo-marketing-candidatures.md` | **Répartition optimale agents — rapports SEO & marketing (candidatures)** : tâches par agent (clusters sémantiques, long-tail, extraction Ads, scénarios trafic). Automatisation Growth, Automatizer, Dev Django. À activer par contact. | **Orchestrateur**, **Chef de Projet**, **Expert SEO**, **Growth**, **Data Analyst**, **Automatizer**, **Dev Django**, **Infographiste**, **Rédacteur** |
| `segmentations/2026-02-09-sprint-general-fitclem-landing-multipage.md` | **Sprint général FitClem** : landing commerciale multipage (candidature Responsable Marketing Digital). Pages : accueil, étude marketing, étude SEO (placeholder), étude iconographique/graphique, proposition KPI ; CTA sur chaque page. **Orchestrateur** : s’assurer que chacun fait sa part (Rédacteur, Designer, Dev Django, Expert SEO, Chef de Projet). Source : `docs/contacts/fitclem/strategie-marketing-fitclem.md`. | **Orchestrateur**, **Chef de Projet**, **Rédacteur**, **Designer**, **Dev Django**, **Expert SEO** |
| `segmentations/2026-01-30-sprint-general-fitclem-generer-page-complete.md` | **Sprint « générer la page complète » FitClem** : landing multipage complète (contenu réel). Étude SEO depuis `rapport-seo-fitclem-manque-a-gagner.md` ; InfographicCraft produit `infographie-manque-a-gagner.html` ; Dev Django routes/vues/templates + intégration. Ordre : structure → Rédacteur → Infographiste → intégration → Expert SEO → validation. | **Orchestrateur**, **Chef de Projet**, **Rédacteur**, **Infographiste**, **Designer**, **Dev Django**, **Expert SEO** |
| `segmentations/2026-01-30-rapport-benchmark-coverage-flux-robots.md` | **Rapport benchmark coverage / flux / robots** : tous les agents exécutent les tests de couverture (`make test-cov-docker`), visent 100 % sur la feature, remplissent le template (métriques coverage, flux n8n/Flowise, actions des robots). Preuves data-driven et premières métriques. | **Tous les agents**, **Architecte**, **Automatizer**, **Pentester**, **Assistant** |
...
- **Date** : 2026-02-15
- **Modifications** : **Création agent Infographiste / Data Viz** (règle `infographiste-dataviz.mdc`) — infographies dynamiques et animées HTML/CSS/JS, effet waouh, pour rapports/landings/présentations ; livrables dans `docs/contacts/<slug>/` ; collaboration Chef de Projet, Designer, Rédacteur, Expert SEO. Registre § 1, § 2, § 3 (ressources Infographiste) mis à jour ; `agents-roles-responsabilites.md` (rôle 11, ligne RACI « Créer infographies dynamiques / data viz ») mis à jour.
- *Précédent* — **Date** : 2026-02-05  
